...
Navigate to the ``chaincode-docker-devmode`` directory
clone:
...
.. code:: bash
  ...
  CORE_PEER_ADDRESS=peer:7052 CORE_CHAINCODE_ID_NAME=mycc:0 ./erc20
...
.. code:: bash
  ...
  peer chaincode install -p chaincodedev/chaincode/token/erc20/go -n myerc -v 0
  peer chaincode instantiate -n myerc -v 0 -c '{"Args":["init","CoinBase","10000000"]}' -C myc

Now issue an invoke to transfer ``10000`` from ``CoinBase`` to ``b``.

.. code:: bash

  peer chaincode invoke -C myc -n myerc -c '{"function":"transfer","Args":["a","20000"]}'
  peer chaincode invoke -C myc -n myerc -c '{"function":"transfer","Args":["b","20000"]}'
  peer chaincode invoke -C myc -n myerc -c '{"function":"transferFrom","Args":["a","b","10000"]}'

  peer chaincode invoke -n mycc -c '{"Args":["invoke","a","b","10"]}' -C myc

Finally, query ``balanceOf``.  We should see values of ``9970000 10000 20000``.

.. code:: bash

  peer chaincode invoke -C myc -n myerc -c '{"Args":["balanceOf","CoinBase"]}'
  peer chaincode invoke -C myc -n myerc -c '{"Args":["balanceOf","a"]}'
  peer chaincode invoke -C myc -n myerc -c '{"Args":["balanceOf","b"]}'


Testing new chaincode
---------------------

By default, we mount only ``token``.  However, you can easily test different
